...
aws ecs describe-services --cluster "${PREFIX}-cluster" --services "${PREFIX}-keycloak" > ecs.json
...
aws ecs describe-services --cluster "${PREFIX}-cluster" --services "${PREFIX}-keycloak" \
  --query 'services[0].{Desired:desiredCount,Running:runningCount,Rollout:deployments[0].rolloutState,MinHealthy:deploymentConfiguration.minimumHealthyPercent,CircuitBreaker:deploymentConfiguration.deploymentCircuitBreaker.rollback}' --output table

TASKS=$(aws ecs list-tasks --cluster "${PREFIX}-cluster" --service-name "${PREFIX}-keycloak" --query 'taskArns' --output text)
aws ecs describe-tasks --cluster "${PREFIX}-cluster" --tasks ${TASKS} \
  --query 'tasks[].{AZ:availabilityZone,Status:lastStatus,Health:healthStatus}' --output table
...
aws logs filter-log-events --log-group-name "/aws/ecs/${PREFIX}/keycloak" \
...
### 1. ECS (consumers first)

```bash
aws application-autoscaling deregister-scalable-target --service-namespace ecs \
  --resource-id "service/${PREFIX}-cluster/${PREFIX}-keycloak" --scalable-dimension ecs:service:DesiredCount

aws ecs update-service --cluster "${PREFIX}-cluster" --service "${PREFIX}-keycloak" --desired-count 0
aws ecs wait services-stable --cluster "${PREFIX}-cluster" --services "${PREFIX}-keycloak"
aws ecs delete-service --cluster "${PREFIX}-cluster" --service "${PREFIX}-keycloak" --force
aws ecs delete-cluster --cluster "${PREFIX}-cluster"
```
...
aws logs delete-log-group --log-group-name "/aws/ecs/${PREFIX}/keycloak"
...
```bash
aws ec2 delete-vpc-endpoints --vpc-endpoint-ids <ids>
