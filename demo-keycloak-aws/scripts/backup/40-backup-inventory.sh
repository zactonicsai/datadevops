...
VPC_ID="$(find_vpc_id)"
CLUSTER="$(cluster_name)"; SERVICE="$(service_name)"; DB_ID="$(db_identifier)"
...
if [[ -n "${VPC_ID}" ]]; then
  ...
  dump vpc-endpoints.json aws ec2 describe-vpc-endpoints --filters "Name=vpc-id,Values=${VPC_ID}"
...
dump ecs-service.json     aws ecs describe-services --cluster "${CLUSTER}" --services "${SERVICE}"
dump ecs-taskdef.json     aws ecs describe-task-definition --task-definition "${NAME_PREFIX}-keycloak"
