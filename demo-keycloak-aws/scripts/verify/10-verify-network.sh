...
if [[ "${NAT_COUNT}" -ge 2 ]]; then
  ...
  check_fail "no NAT Gateways - private tasks cannot pull images"
fi
...
# --- VPC endpoints ----------------------------------------------------------
EP_COUNT="$(aws ec2 describe-vpc-endpoints --filters "Name=vpc-id,Values=${VPC_ID}" \
  --query 'length(VpcEndpoints[?State==`available`])' --output text)"
[[ "${EP_COUNT}" -ge 4 ]] \
  && check_pass "VPC endpoints available: ${EP_COUNT}" \
  || check_fail "VPC endpoints available: ${EP_COUNT} (expected 5: s3, secretsmanager, ecr.api, ecr.dkr, logs)"
