...
| `stdout_callback = ansible.builtin.default` | Use the built-in Ansible screen output plugin |
| `callback_result_format = yaml` | Show results in easier-to-read YAML style |
| `pipelining = True` | Make SSH work faster |

Important: older tutorials used `stdout_callback = yaml`. Newer Ansible/community.general versions can fail with that setting, so this lab uses the newer built-in setting.
