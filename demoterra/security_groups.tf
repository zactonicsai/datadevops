...
# ----------------------------------------------------------------------------
# Optional SSH access
# Empty by default - prefer SSM Session Manager over opening port 22.
# ----------------------------------------------------------------------------
...
# Instances also need plain HTTP out for the Docker image pull and yum/dnf.
resource "aws_vpc_security_group_egress_rule" "keycloak_http_out" {
  security_group_id = aws_security_group.keycloak.id
  description       = "HTTP out for package and image downloads"
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 80
  to_port           = 80
  ip_protocol       = "tcp"
